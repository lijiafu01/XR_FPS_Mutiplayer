...
public class PlayerTeleporter : MonoBehaviour
{
    ...
    private void Start()
    {
        Player_CharacterController = bodyTransforn.gameObject.GetComponent<CharacterController>();
    }
    ...
    public void Teleport()
    {
        if (groundDetected)
        {
            float halfHeight = Player_CharacterController.height / 2.0f;
            bodyTransforn.position = groundPos + lastNormal * 0.1f + Vector3.up * halfHeight;
        }
        else
        {
